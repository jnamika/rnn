...
def generate_logistic_map(length, transient=0, a=4.0, x=None):
    if (x == None):
        x = random.random()
    for n in xrange(length + transient):
        if (n >= transient):
...
def generate_henon_map(length, transient=0, a=1.4, b=0.3, x=None, y=None):
    if (x == None):
...
def generate_Lorenz_attractor(length, transient=0, p=10, r=28, b=8.0/3.0,
...
def generate_van_der_Pol_attractor(length, transient=0, a=1.0, b=0.25,
...
def generate_composite_dynamics(length, f, v, transient=0, p_matrix=None,
...
def generate_golden_mean_shift(length, transient=0, probability=0.5):
    c = 0
    for n in xrange(length + transient):
        if (c == 0):
            p = random.random()
...
def generate_01x_shift(length, transient=0, probability=0.5):
    for n in xrange(length + transient):
        c = n % 3
        if (c == 2):
            p = random.random()
...
def generate_Morse_shift(length, transient=0):
    s = [0]
    for n in xrange(length + transient):
        if (n == len(s)):
...
def print_logistic_map(length, transient=0, a=4.0, x=None):
    for x in generate_logistic_map(length, transient, a, x):
        x = 0.8 * (2 * x - 1)
        print '%f' % x


def print_henon_map(length, transient=0, a=1.4, b=0.3, x=None, y=None):
    for x, y in generate_henon_map(length, transient, a, b, x, y):
        x = 0.625 * x
        print '%f\t%f' % (x, y)


def print_sin_curve(length, period, transient=0):
    for v in generate_Tn_torus(length=length,
            parameters=[(0,(2*math.pi)/period,0.8,0)], transient=transient):
        print '\t'.join([str(x) for x in v])


def print_Lissajous_8curve(length, period, transient=0):
    for v in generate_Tn_torus(length=length,
            parameters=[(0,(4*math.pi)/period,0.8,0),
                ((0,(2*math.pi)/period,0.8,0))], transient=transient):
        print '\t'.join([str(x) for x in v])


def print_Lorenz_attractor(length, tau=1, transient=0):
...
def print_van_der_Pol_attractor(length, a=1, transient=0):
...
def print_comp_Lissajous_9curves(length, period, transient=0):
    p4 = period / 4.0
    p = period
    ...
    for v in generate_composite_dynamics(length=length, f=f,
            ...
            p_matrix=p_matrix, init_f=0, transient=transient):
        print '\t'.join([str(x) for x in v[:2]])



def print_golden_mean_shift(length, transient=0, probability=0.5,
        output_type='tanh'):
    for x in generate_golden_mean_shift(length, transient, probability):
        if (output_type == 'tanh'):
...
def print_01x_shift(length, transient=0, probability=0.5, output_type='tanh'):
    for x in generate_01x_shift(length, transient, probability):
        if (output_type == 'tanh'):
            print "%d" % 1 if x == 1 else -1
        elif (output_type == 'softmax'):
            y = 0 if x == 1 else 1
...
def print_Morse_shift(length, transient=0, output_type='tanh'):
    for x in generate_Morse_shift(length, transient=transient):
        if (output_type == 'tanh'):
            print "%d" % 1 if x == 1 else -1
        elif (output_type == 'softmax'):
            y = 0 if x == 1 else 1
...
def main():
    ...
    print_Lorenz_attractor(1000,transient=1000)
    print_van_der_Pol_attractor(1000,transient=1000)
